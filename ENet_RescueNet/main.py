...
def load_dataset(dataset):
    ...
    if args.mode.lower() == 'test':
        images, labels, _ = iter(test_loader).next()
    else:
    ...
    # Show a batch of samples and labels/ Save test set input masks in color
    if not os.path.exists('savedlabels'):
        os.makedirs('savedlabels')
    if args.imshow_batch:
        for _, batch_data in enumerate(test_loader):
            images = batch_data[0]
            labels = batch_data[1]
            label_paths = batch_data[2]
            ...
            im_paths = label_paths
            for color_label, impath in zip(color_labels, im_paths):
                outname = os.path.splitext(impath)[0] + '.png'
                outname_final = os.path.join(args.savelabel, outname)
                save_image(color_label, outname_final)
            
        print("Done. Saving the color maps of test input masks!")
...
def train(train_loader, val_loader, class_weights, class_encoding):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # @sh: add to generate graphs for loss and accuracy
        train_epochs.append(epoch)
        train_loss.append(epoch_loss)
        train_accuracy.append(miou)
    ...
    filename_tr_acc = "ENet-harvey-feb13-2021-2-train-accuracy.csv"
    filename_val_acc = "ENet-harvey-feb13-2021-2-val-accuracy.csv"
    filename_tr_loss = "ENet-harvey-feb13-2021-2-train-loss.csv"
    filename_val_loss = "ENet-harvey-feb13-2021-2-val-loss.csv"
...
def test(model, test_loader, class_weights, class_encoding):
    ...
    # Show a batch of samples and labels/ Save predicted masks of test set in color
    ##-- /begin: @sh: modified imshow_batch
    if args.imshow_batch:
    ...
    ##-- /end: @sh: modified imshow_batch

## @sh: modified version of actual predict function.
def predict(model, images, paths, class_encoding):
    ...
    im_paths = paths
    for predict, impath in zip(color_predictions, im_paths):
        outname = os.path.splitext(impath)[0] + '.png'
...
if __name__ == '__main__':
    ...
    elif args.dataset.lower() == 'uavtest':
        from data import UAVTest as dataset
    elif args.dataset.lower() == 'michaeltest':
        from data import MichaelTest as dataset
    elif args.dataset.lower() == 'harvey':
        from data import Harvey as dataset
    elif args.dataset.lower() == 'rescuenet':
